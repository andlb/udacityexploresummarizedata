...
Until now, the analyse was about the estimated loss. The next plot will tell the amount of default loan by LP_NetPrincipalLoss, what means principal that remains uncollected after any recoveries. 
...
ggplot(aes(ProsperRating..Alpha., LP_NetPrincipalLoss),data = prosper_loan_default) +
  ...
  scale_x_discrete(limits = c('AA', 'A', 'B', 'C', 'D', 'E','HR')) +
  labs(x = "Prosper rating from AA (lower risk, lower return) to HR (higher risk, higher return).", y = 'Count of default loans') 

```

The riskier is the loan, the concentration of default loan increase.
...
Before the investor put their money in an unsecure investiment they need to understand the estimated return, estimated loss and the features which truly cause the investiment return or loss.
Prosper dataset store data from the lower and upper credit score  provided by a consumer credit rating agency and after July 2009, prosper loan built a custom risk score using its historical data. 

There is a strong relation between the prosper rating and the estimated return and estimated loss. The same does not happen with the relation between the cusumer credit rating and the estimated loss and estimated return.

After investigate the estimated return and loss, the investiagation focused in the relation between the prosper rating and the default loans. Based on the concentration of point, we figured out the higher is the risk, higher is the concentration of default loans. However, it is perceptible the amount loan limit. The reason is the higher prosper risk the lower is the amount the borrower can get.

Investigating feature income range, I figured out the higher is the income range the lower is the estimated loss. Using box plot, they shown as the income range increase the first quartile, third quartile and the mean from estimated loss feature reduced.

However, the most strong relation is between the prosper rating and the estimated loss. 

# Multivariate Plots Section

After investigate the relation between estimated loss and income range, a similar distribution was found between estimated loss and prosper rating feature.It was like the income range had a strong weight in the prosper rating definition.

The next plot, these 3 features are going to be shown at the same plot.

```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(IncomeRange, EstimatedLoss), data = prosper_loan_temp) +
    geom_point(aes(color = ProsperRating..Alpha., fill = ProsperRating..Alpha.), position = 'jitter', alpha = 0.05) +
    scale_x_discrete(limits = c('Not employed', '$1-24,999', '$25,000-49,999', '$50,000-74,999', '$75,000-99,999', '$100,000+')) +
...
``` 
The plot show a clear division from the prosper rating by estimated loss. However, this division is proportional for all income range, what means, there isn't a strong influence from the prosper rating on the income range.
```{r echo=FALSE, message=FALSE, warning=FALSE}
